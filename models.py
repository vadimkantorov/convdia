...
class SincConv1d(nn.Conv1d):
	def __init__(
		...
		**kwargs,
		...
		self.band_hz_ = nn.Parameter(ediff1d(hz).view(-1, 1))
	...
	def weight(self):
		...
		high = (low + self.min_band_hz + torch.abs(self.band_hz_)).clamp(min = self.min_low_hz, max = self.sample_rate / 2)
		band = (high - low)[:, 0]
		...
		return band_pass.view(self.out_channels, 1, -1)
...
class TDNN(nn.Module):
	def __init__(
		...
		self.temporal_conv = torch.nn.utils.weight_norm(nn.Conv1d(in_channels, out_channels, kernel_size, dilation = dilation))
...
class XVectorNet(nn.Module):
	def __init__(self, input_dim: int = 24, embedding_dim: int = 512):
		...
		frame1 = TDNN(in_channels=input_dim, out_channels=512, context=[-2, 2   ], full_context=True)
		frame2 = TDNN(in_channels=      512, out_channels=512, context=[-2, 0, 2], full_context=False)
		frame3 = TDNN(in_channels=      512, out_channels=512, context=[-3, 0, 3], full_context=False)
		frame4 = TDNN(in_channels=      512, out_channels=512, context=[0       ], full_context=True)
		frame5 = TDNN(in_channels=      512, out_channels=1500,context=[0       ], full_context=True)
		self.tdnn = nn.Sequential(frame1, frame2, frame3, frame4, frame5, StatsPool())
		self.segment6 = nn.Linear(2*1500, embedding_dim)
